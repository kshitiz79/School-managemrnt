...
const ExpenseHead = () => {
  ...
  const addMutation = useMutation({
    mutationFn: financeApi.addExpenseHead,
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => financeApi.updateExpenseHead(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries(['expenseHeads'])
      setFormData({ name: '', description: '' })
      setEditingId(null)
  ...
  const deleteMutation = useMutation({
    mutationFn: financeApi.deleteExpenseHead,
    onSuccess: () => {
      queryClient.invalidateQueries(['expenseHeads'])
  ...
  return (
    <div className="p-6">
      <div className="bg-white rounded-lg shadow-md p-6">
        <h2 className="text-2xl font-bold mb-6">Expense Heads Management</h2>
        ...
        <div className="bg-gray-50 p-4 rounded-md mb-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-4">
              ...
              >
                {editingId ? 'Update' : 'Add'} Expense Head
              </Button>
              {editingId && (
                <Button
                  type="button"
        ...
        {expenseHeads.length === 0 && !isLoading && (
          <div className="text-center py-8 text-gray-500">
            No expense heads found. Add one to get started.
          </div>
        )}
      </div>
    </div>
